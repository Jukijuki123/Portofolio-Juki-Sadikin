  ...
  <div className="flex flex-wrap bg-white rounded-xl p-1 shadow-sm w-full md:w-fit mb-8 gap-1">
    {["work", "education"].map((tab) => (
      <button
        ...
        className={`flex-1 md:flex-none px-4 py-2 rounded-lg text-xs md:text-sm font-medium transition-all duration-300 ${
          activeTab === tab
      ...
      >
        {data.map((item, index) => (
          <div key={index} className="relative border-l-2 border-gray-200 pl-5 py-1 w-full ml-2">
            ...
            <div className="absolute -left-[9px] top-2 w-4 h-4 rounded-full bg-primary border-4 border-white shadow-sm shrink-0" />
            
            <h4 className="font-space font-bold text-base md:text-lg text-gray-900 break-words">
            ...
            <div className="flex flex-wrap gap-2 mt-2 mb-2">
              ...
              <span className="text-[10px] md:text-xs text-gray-500 flex items-center break-words">
                {activeTab === "work" ? item.location[lang] : item.major[lang]}
              </span>
            </div>

            {item.role && (
              <p className="font-medium text-gray-800 text-xs md:text-sm mb-2 break-words">
                {item.role[lang]}
              </p>
            )}
              ...
              <ul className="text-gray-600 space-y-1 text-xs md:text-sm leading-relaxed">
                {item.points.map((p, i) => (
                  <li key={i} className="flex gap-2 items-start">
                    ...
                    <span className="break-words">{p[lang]}</span>
                  </li>
                ))}
              </ul>
            )}
